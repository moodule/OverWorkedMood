...
lint: ## check style with flake8
	$(ENVIRONMENT) flake8 overworked tests
...
coverage: ## check code coverage quickly with the default Python
	$(ENVIRONMENT) coverage run --source overworked -m pytest
	$(ENVIRONMENT) coverage report -m
	$(ENVIRONMENT) coverage html
	$(BROWSER) htmlcov/index.html

docs: ## generate Sphinx HTML documentation, including API docs
	rm -f docs/overworked.rst
	rm -f docs/modules.rst
	$(ENVIRONMENT) sphinx-apidoc -o docs/ overworked
	$(MAKE) -C docs clean
	$(MAKE) -C docs html
